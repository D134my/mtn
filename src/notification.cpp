...
void notify::notification(int hour, int min, std::string_view task,
                          std::string const &music_path) {

  std::cout << "task -> " << task << "will finish at " << hour << ':' << min
            << std::endl;
  ...
  std::string command_line_msg = "task " + std::string(task) + " finished at " +
                                 std::to_string(hour) + ":" +
                                 std::to_string(min);
  std::cout << command_line_msg << std::endl;
